...
export async function createUsedGoodsPostRecord(input: {
  ...
  if (await containsProfanity([input.title, input.content].join(" "))) {
  ...
  const { data: post, error: postError } = await supabaseAdmin
    .from("posts")
    .insert({ user_id: publicUser.id, post_type: "used_goods", title: input.title, status: "active", view_count: 0, like_count: 0 })
    .select("id")
    .single();
