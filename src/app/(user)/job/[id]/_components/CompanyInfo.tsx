...
export default function CompanyInfo({ job }: { job: JobDetail }) {
  ...
  return (
    <div>
      <h2 className="text-xs font-semibold tracking-[0.15em] uppercase text-gray-600 mb-4">{t("companyInfo")}</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
        {job.company_year && (
          <div>
            ...
            <p className="text-sm font-medium text-gray-900">{t("yearSuffix", { year: job.company_year })}</p>
          </div>
        )}
        {job.employee_count && (
          <div>
            <p className="text-xs text-gray-400">{t("employeeCount")}</p>
            <p className="text-sm font-medium text-gray-900">{t("employeeCountValue", { count: job.employee_count })}</p>
          </div>
        )}
        {job.company_address && (
          <div>
            <p className="text-xs text-gray-400">{t("address")}</p>
            <p className="text-sm font-medium text-gray-900">{job.company_address}</p>
          </div>
        )}
        {job.company_website && (
          <div>
