...
export const REASON_STYLE: Record<string, { fg: string; bg: string }> = {
  ...
  비방: { fg: "#f97316", bg: "#fff7ed" },
  사기: { fg: "#dc2626", bg: "#fef2f2" },
  허위: { fg: "#8b5cf6", bg: "#f5f3ff" },
  불법: { fg: "#b91c1c", bg: "#fef2f2" },
  "성적 콘텐츠": { fg: "#be185d", bg: "#fdf2f8" },
...
export function normalizeReportReason(category: string): string {
  const c = category.trim();
  if (!c) return "기타";
  if (c.includes("욕설") || c.includes("비방")) return "욕설";
  if (c.includes("사기")) return "사기";
  if (c.includes("허위")) return "허위";
  if (c.includes("불법")) return "불법";
  if (c.includes("성") || c.includes("음란") || c.includes("추행")) {
    return "성적 콘텐츠";
  }
  return c;
}
