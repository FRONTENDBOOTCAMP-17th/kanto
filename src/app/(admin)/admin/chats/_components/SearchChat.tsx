...
export default function SearchChat({ chats }: { chats: ChatRoom[] }) {
  ...
  const sel =
    selId != null ? (chats.find((c) => c.id === selId) ?? null) : null;
    ...
    <>
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="whitespace-nowrap rounded-[11px] border border-[#e7ebee] bg-white px-3.5 py-2.25 text-[13px] font-medium text-slate-500">
          총 <span className="font-bold text-slate-900">{chats.length}</span>개
          채팅방
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-[18px] border border-[#e7ebee] bg-white shadow-[0_1px_2px_rgba(16,24,40,0.04)]">
            ...
            <tbody>
              {pageItems.map((chat) => (
                ...
                >
                  ...
                  <td className="px-4.5 py-3.75">
                    {chat.posts ? (
                      <div className="flex items-center gap-1.5">
                        <span className="inline-flex shrink-0 items-center whitespace-nowrap rounded-md bg-slate-100 px-2 py-0.5 text-[11.5px] font-semibold text-slate-600">
                          {POST_TYPE_LABEL[chat.posts.post_type] ??
                            chat.posts.post_type}
                        </span>
                        <span className="max-w-35 truncate text-[13.5px] text-slate-700">
                          {chat.posts.title}
                        </span>
                      </div>
                    ) : (
                  ...
                  <td className="max-w-50 px-4.5 py-3.75">
        ...
        {pageItems.length > 0 && (
          <div className="lg:hidden divide-y divide-[#f3f5f7]">
            {pageItems.map((chat) => {
              const c = chat.posts
                ? POST_TYPE_COLOR[chat.posts.post_type]
                : null;
              return (
      ...
      {selId != null && sel && (
        <>
          ...
          />
          <div
            className="fixed right-0 top-0 z-71 flex h-screen w-130 max-w-full flex-col bg-white shadow-[-12px_0_44px_rgba(15,23,42,0.18)]"
            ...
            <div className="border-b border-[#f1f4f6] px-6 py-4">
              {sel.posts ? (
                <div className="mb-2 flex items-center gap-2 flex-wrap">
                  {(() => {
                    const c = POST_TYPE_COLOR[sel.posts.post_type] ?? {
                      bg: "#f1f5f9",
                      fg: "#64748b",
                    };
                    return (
                      <span
                        className="inline-flex items-center rounded-full px-2.75 py-1 text-[12px] font-bold"
                        style={{ background: c.bg, color: c.fg }}
                      >
                        {POST_TYPE_LABEL[sel.posts.post_type] ??
                          sel.posts.post_type}
                      </span>
                    );
                  })()}
            ...
            <div className="flex gap-2 border-b border-[#f1f4f6] px-6 py-4">
              {[
                ...
                <button
                  ...
                  className={[
                    "flex-1 whitespace-nowrap rounded-[10px] py-2.5 text-[13px] font-bold transition-colors",
                    tab === key
                      ? key === "chat"
              ...
              {(tab === "user1" || tab === "user2") &&
                (() => {
                  ...
                  const effectiveSuspended =
                    user.id in localSuspended
                      ? localSuspended[user.id]
                      : user.suspended_until;
                  const isSuspended =
                    !!effectiveSuspended &&
                    new Date(effectiveSuspended) > new Date();
                    ...
                    <div className="flex flex-col gap-4">
                      <div className="rounded-[14px] border border-[#eef1f3] bg-white p-4">
                        <div className="flex items-center justify-between">
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-1 rounded-[8px] border border-[#e7ebee] bg-slate-50 px-3 py-1.75 text-[12.5px] font-semibold text-slate-600 hover:bg-slate-100"
                          >
                            상세 보기{" "}
                            <ExternalLink
                        ...
                        {isSuspended && (
                          <div className="mt-2 rounded-[8px] bg-red-50 px-3 py-2 text-[12px] font-semibold text-red-600">
                            정지 중 · 만료:{" "}
                            {effectiveSuspended === "9999-12-31T00:00:00Z"
                              ? "영구"
                              : effectiveSuspended?.slice(0, 10)}
                          </div>
                        )}
                        ...
                        <div className="mt-3 flex flex-wrap gap-1.5">
                          <button
                            ...
                            className="rounded-[8px] border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-[12px] font-bold text-emerald-700 hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-40"
                          >
                            제재 해제
                          </button>
                          {(["7d", "30d", "perm"] as const).map((t) => (
                            <button
                              ...
                              disabled={isPending}
                              className="rounded-[8px] border border-red-200 bg-red-50 px-3 py-1.5 text-[12px] font-bold text-red-600 hover:bg-red-100 disabled:opacity-40"
                            >
                              {t === "7d"
                                ? "7일 정지"
                                : t === "30d"
                      ...
                      {!chatLoading && filtered.length > 0 && (
                        <div className="rounded-[14px] border border-[#eef1f3] bg-white">
                          ...
                          <div className="max-h-90 divide-y divide-[#f5f7f8] overflow-y-auto">
                            {filtered.map((msg) => (
                              <div
                                key={msg.id}
                              ...
                              >
                                ...
                                <span className="break-all text-[13px] leading-relaxed text-slate-700">
                                  {msg.content}
                                </span>
                              </div>
                            ))}
                ...
                <>
                  ...
                  {!chatLoading && messages.length > 0 && (
                    <div className="flex flex-col gap-2">
                      ...
                      {!hasMore && (
                        <div className="py-2 text-center text-[11.5px] text-slate-300">
                          — 처음 메시지입니다 —
                        </div>
                      )}
                      {messages.map((msg, index) => {
                        ...
                        return (
                          <Fragment key={msg.id}>
                            ...
                            <div
                              className={`flex flex-col gap-0.5 ${isUser1 ? "items-start" : "items-end"}`}
                            >
                              <span className="px-1 text-[11px] text-slate-400">
                                {msg.sender?.name ?? "-"}
                              </span>
                              <div
                                className={`flex items-end gap-1 ${isUser1 ? "" : "flex-row-reverse"}`}
                              >
                                <div
                                  className={`max-w-[72%] break-keep rounded-2xl px-3 py-2 text-sm leading-relaxed ${isUser1 ? "rounded-tl-sm bg-white text-gray-800 shadow-sm" : "rounded-tr-sm bg-teal-500 text-white"}`}
                                >
                                  {msg.content}
                                </div>
              ...
              <div className="border-t border-[#f1f4f6] px-6 py-4.5">
                <div className="flex items-center justify-between">
                  <span className="text-[13px] text-slate-400">
                    ...
                    개
                    {hasMore && (
                      <span className="ml-1 text-slate-300">(더 있음)</span>
                    )}
                  </span>
                  <button
                    onClick={() => {
                    ...
                    className="inline-flex items-center gap-1.5 rounded-[9px] border border-[#e2e8eb] bg-white px-3 py-1.75 text-[12.5px] font-semibold text-slate-600 hover:bg-slate-50"
                  >
                    <svg
                      width="14"
                      height="14"
