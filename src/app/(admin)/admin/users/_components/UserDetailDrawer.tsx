...
export default function UserDetailDrawer({ user, onClose, onChanged, onToast }: Props) {
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 z-71 flex h-screen w-115 max-w-full flex-col bg-white shadow-[-12px_0_44px_rgba(15,23,42,0.18)]" style={{ animation: "drawerIn .26s cubic-bezier(.4,0,.2,1)" }}>
        ...
        <div className="flex items-center justify-between gap-3 border-b border-[#f1f4f6] px-6 py-5.5">
          ...
          <button onClick={onClose} className="flex h-8.5 w-8.5 items-center justify-center rounded-[9px] border border-[#eef1f3] bg-white text-slate-500 hover:bg-slate-100">
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain p-6">
          <div className="mb-5 flex flex-wrap items-center gap-2">
            {isSuspended ? (
              <span className="inline-flex items-center whitespace-nowrap rounded-full px-2.75 py-1 text-[12px] font-bold" style={{ background: "#fef2f2", color: "#dc2626" }}>
                {isPerm ? "영구 정지" : "정지 중"}
              </span>
            ) : (
              <span className="inline-flex items-center whitespace-nowrap rounded-full px-2.75 py-1 text-[12px] font-bold" style={{ background: "#ecfdf5", color: "#059669" }}>
                정상
              </span>
            )}
            {user.report_count > 0 && (
              <span className="inline-flex items-center whitespace-nowrap rounded-full px-2.75 py-1 text-[12px] font-semibold" style={{ background: "#fef2f2", color: "#dc2626" }}>
          ...
          {user.pending_report_count > 0 && (
            <div className="mb-5 rounded-[14px] border border-red-100 bg-red-50 p-3.5">
              <div className="flex items-center gap-1.5 text-[12px] font-bold uppercase tracking-wide text-red-500">
                <AlertTriangle className="h-3.75 w-3.75" strokeWidth={2.2} />
                처리 대기 신고 {user.pending_report_count}건
              </div>
              <div className="mt-2.5 flex flex-col gap-2">
                {user.pending_reports.map((r, i) => (
                  <div key={i} className="flex items-start gap-2">
                    <span className="mt-0.5 inline-flex shrink-0 items-center whitespace-nowrap rounded-full bg-white px-2 py-0.5 text-[11.5px] font-bold text-red-600">
                      {r.reason}
                    </span>
                    {r.description && (
          ...
          <div className="mb-5 rounded-[14px] border border-[#eef1f3] bg-slate-50 p-4.5">
            <div className="mb-3 flex items-center gap-1.5 text-[12px] font-bold uppercase tracking-wide text-slate-400">
              <Users className="h-4.25 w-4.25 text-teal-600" strokeWidth={2} />
              회원
            </div>
            ...
            {sanctions.length === 0 ? (
              ...
              <div className="divide-y divide-[#f3f5f7]">
                {pagedSanctions.map((s) => {
                  ...
                  return (
                    <div key={s.id} className="px-4 py-3.25">
            ...
            <div>
                  ...
                  {SANCTION_OPTIONS.map((s) => {
                    ...
                    return (
                      <button
                        ...
                        className="flex-1 whitespace-nowrap rounded-[9px] border py-2.25 text-center text-[12.5px] font-bold"
                        style={
                          selected
                            ? optPerm
              ...
              <div className="flex gap-2.5">
                ...
                <button
                  ...
                  disabled={isPending}
                  className="flex-[1.4] rounded-[11px] bg-red-500 py-3.25 text-[14px] font-bold text-white shadow-[0_4px_12px_rgba(239,68,68,0.3)] hover:bg-red-600 disabled:opacity-60"
                >
            ...
            <div className="flex flex-col gap-2">
              <div className="flex gap-2.5">
                <button
                  ...
                  disabled={isPending}
                  className="flex-1 rounded-[11px] border border-red-200 bg-red-50 py-3.25 text-[14px] font-bold text-red-600 hover:bg-red-100 disabled:opacity-60"
                >
                  제재 부여
                </button>
                <button
              ...
              {isSuspended && (
                <button
                  ...
                  disabled={isPending}
                  className="w-full rounded-[11px] border border-emerald-200 bg-emerald-50 py-2.75 text-[13.5px] font-bold text-emerald-700 hover:bg-emerald-100 disabled:opacity-60"
                >
