...
const MOCK_TEAMS: Team[] = [
  { id: 1, name: "운영팀", permissions: ["handle_report", "sanction_user", "view_chat"], createdAt: "2026-01-15" },
  { id: 2, name: "콘텐츠팀", permissions: ["delete_post", "write_notice"], createdAt: "2026-02-01" },
];

const MOCK_ADMINS: AdminAccount[] = [
  { id: 1, nickname: "슈퍼관리자", email: "super@kanto.io", role: "super_admin", teamId: null, createdAt: "2026-01-01" },
  { id: 2, nickname: "운영팀A", email: "ops-a@kanto.io", role: "admin", teamId: 1, createdAt: "2026-03-10" },
  { id: 3, nickname: "운영팀B", email: "ops-b@kanto.io", role: "admin", teamId: 1, createdAt: "2026-04-05" },
  { id: 4, nickname: "콘텐츠팀A", email: "content-a@kanto.io", role: "admin", teamId: 2, createdAt: "2026-04-22" },
];

const MOCK_USER_POOL: UserResult[] = [
  { id: 101, nickname: "마닐라고양이", email: "cat@manila.ph", joinedAt: "2025-11-03" },
  { id: 102, nickname: "세부여행자", email: "traveler@cebu.ph", joinedAt: "2025-12-18" },
  { id: 103, nickname: "민다나오별", email: "star@mindanao.ph", joinedAt: "2026-01-07" },
  { id: 104, nickname: "보라카이서퍼", email: "surf@boracay.ph", joinedAt: "2026-02-14" },
  { id: 105, nickname: "팔라완탐험가", email: "explore@palawan.ph", joinedAt: "2026-03-22" },
  { id: 106, nickname: "다바오농부", email: "farm@davao.ph", joinedAt: "2026-04-01" },
  { id: 107, nickname: "일로일로맛집", email: "food@iloilo.ph", joinedAt: "2026-05-09" },
];
...
function UserSearchModal({
  ...
  const filtered = MOCK_USER_POOL.filter(
    (u) => !excludeIds.has(u.id) && (u.nickname.includes(query) || u.email.includes(query)),
  );
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md rounded-2xl border border-[#ebeef0] bg-white shadow-[0_8px_40px_rgba(0,0,0,0.14)]"
            ...
            filtered.map((u) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[13.5px] font-semibold text-slate-800">{u.nickname}</p>
          ...
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center gap-2 py-10 text-slate-300">
              ...
              <p className="text-[13.5px]">
                {query ? "검색 결과가 없습니다." : teams.length === 0 ? "등록된 팀이 없습니다." : "검색어를 입력하세요."}
              </p>
            </div>
          ) : (
            filtered.map((t) => (
...
export default function PermissionsPage() {
  ...
  function handleAddTeam() {
    ...
    if (!name) return;
    const now = new Date().toISOString().slice(0, 10);
    setTeams((prev) => [...prev, { id: Date.now(), name, permissions: [], createdAt: now }]);
    setTeamNameInput("");
    setAddingTeam(false);
  }
  ...
  function toggleTeamPermission(teamId: number, perm: AdminPermission) {
    setTeams((prev) =>
      prev.map((t) => {
        ...
        const has = t.permissions.includes(perm);
        return {
          ...t,
          permissions: has ? t.permissions.filter((p) => p !== perm) : [...t.permissions, perm],
        };
      }),
    );
    // keep selectedTeamDetail in sync
  ...
  function AdminRow({ admin, isLast }: { admin: AdminAccount; isLast: boolean }) {
    ...
    return (
      ...
      >
          ...
          <div className="flex items-center gap-3">
            <div
              className={[
                ...
                isSuperAdmin ? "bg-amber-100" : "bg-teal-50",
              ].join(" ")}
            >
              {isSuperAdmin ? (
            ...
            <div className="min-w-0">
              <p className="font-semibold text-slate-800">{admin.nickname}</p>
        ...
        <td className="px-5 py-4">
          {isSuperAdmin ? (
            <span className="rounded-full border border-amber-200 bg-amber-50 px-2.5 py-0.5 text-[12px] font-semibold text-amber-600">
              전체 권한
            </span>
          ) : team ? (
            <span className="rounded-full border border-teal-200 bg-teal-50 px-2.5 py-0.5 text-[12px] font-semibold text-teal-600">
              {team.name}
            </span>
          ) : (
        ...
        <td className="px-5 py-4 text-slate-500 whitespace-nowrap">{admin.createdAt}</td>
        <td className="px-5 py-4">
          {!isSuperAdmin &&
            (pendingDelAdminId === admin.id ? (
              <div className="flex items-center justify-end gap-1">
                <button
                  ...
                  className="rounded-lg px-2.5 py-1.5 text-[12px] font-semibold text-red-500 hover:bg-red-50"
                >
                  삭제
                </button>
                <button
                  onClick={() => setPendingDelAdminId(null)}
              ...
              <div className="flex justify-end">
                <button
                  onClick={() => setPendingDelAdminId(admin.id)}
  ...
  function GroupHeader({ label, count }: { label: string; count: number }) {
    return (
      <tr>
        <td colSpan={4} className="border-b border-[#ebeef0] bg-slate-50 px-5 py-2">
          <div className="flex items-center gap-2">
      ...
      <div className="p-6 lg:p-8">
        ...
        <div className="mb-7">
          ...
          <p className="mt-1 text-[13px] text-slate-500">
            팀 단위로 권한을 설정하고 어드민 계정을 관리합니다. 슈퍼어드민 전용 페이지입니다.
          </p>
        </div>
          ...
          <div className="flex w-fit gap-1 rounded-xl border border-[#ebeef0] bg-slate-50 p-1">
            {(
              ...
              <button
                ...
                className={[
                  "flex items-center gap-1.5 rounded-lg px-4 py-2 text-[13.5px] font-semibold transition-colors",
            ...
            <table className="w-full text-[13.5px]">
              <thead>
                <tr className="border-b border-[#ebeef0] text-left text-[12px] font-semibold uppercase tracking-wide text-slate-400">
                  ...
                  <th className="px-5 py-3.5 whitespace-nowrap">추가일</th>
              ...
              <tbody>
                {superAdmins.length > 0 && (
                  <>
                    <GroupHeader label="슈퍼어드민" count={superAdmins.length} />
                    {superAdmins.map((a, i) => (
                      <AdminRow key={a.id} admin={a} isLast={i === superAdmins.length - 1 && teams.length === 0 && unassigned.length === 0} />
                    ))}
                  </>
                )}
                {teams.map((team) => {
                  ...
                  return (
                    <>
                      <GroupHeader key={`hd-${team.id}`} label={team.name} count={members.length} />
                      {members.length === 0 ? (
                        <tr key={`empty-${team.id}`}>
                          <td colSpan={4} className="border-b border-[#ebeef0] px-5 py-5 text-center text-[13px] text-slate-300">
                            소속 어드민이 없습니다.
                          </td>
                        </tr>
                      ) : (
                        members.map((a, i) => (
                          <AdminRow key={a.id} admin={a} isLast={i === members.length - 1} />
                        ))
                      )}
                    </>
                  );
                ...
                {unassigned.length > 0 && (
                  <>
                    <GroupHeader label="미배정" count={unassigned.length} />
                    {unassigned.map((a, i) => (
                      <AdminRow key={a.id} admin={a} isLast={i === unassigned.length - 1} />
                    ))}
                  </>
                )}
              </tbody>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[12.5px] text-slate-400">
                        멤버 {members.length}명 · {selectedTeamDetail.createdAt} 생성
                      </p>
                    </div>
                  </div>
                        ...
                        return (
                          <button
                            ...
                            className={[
                              ...
                              active ? "border-teal-400 bg-teal-50" : "border-[#ebeef0] hover:bg-slate-50",
                            ].join(" ")}
                          >
                            <span
                              className={[
                                ...
                                active
                                  ...
                                  : "border-[#dde1e4] bg-white",
                              ].join(" ")}
                            >
                              {active && <Check className="h-3 w-3 text-white" strokeWidth={2.5} />}
                            </span>
                            <span className="min-w-0">
                              <span className={["block text-[13px] font-semibold", active ? "text-teal-700" : "text-slate-700"].join(" ")}>
                                {p.label}
                              </span>
                        ...
                        <tbody>
                          {members.map((member, i) => (
                            <tr
                              ...
                              className={[
                                "transition-colors hover:bg-slate-50",
                                i !== members.length - 1 ? "border-b border-[#ebeef0]" : "",
                              ].join(" ")}
                            >
                              <td className="px-5 py-4">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div className="min-w-0">
                                    <p className="font-semibold text-slate-800">{member.nickname}</p>
                                    <p className="truncate text-[12px] text-slate-400">{member.email}</p>
                                  </div>
                                </div>
                              </td>
                              <td className="px-5 py-4 whitespace-nowrap text-slate-500">{member.createdAt}</td>
                              <td className="px-5 py-4">
                                {pendingExcludeId === member.id ? (
                                  ...
                                  <div className="flex justify-end">
                                    <button
                                      ...
                                      className="rounded-lg border border-[#ebeef0] px-3 py-1.5 text-[12px] font-medium text-slate-400 hover:border-red-200 hover:bg-red-50 hover:text-red-400"
                                    >
                                      팀에서 제외
                                    </button>
                                  </div>
                  ...
                  <table className="w-full text-[13.5px]">
                    <thead>
                      <tr className="border-b border-[#ebeef0] text-left text-[12px] font-semibold uppercase tracking-wide text-slate-400">
                        ...
                        <th className="px-5 py-3.5 whitespace-nowrap">생성일</th>
                        <th className="px-5 py-3.5" />
                      </tr>
                    </thead>
                    <tbody>
                      {teams.map((team, i) => {
                        ...
                        return (
                          <tr
                            ...
                            className={[
                              "cursor-pointer transition-colors hover:bg-teal-50",
                              !isLast ? "border-b border-[#ebeef0]" : "",
                            ].join(" ")}
                          >
                            <td className="px-5 py-4">
                                ...
                                {team.permissions.length === 0 ? (
                                  ...
                                  team.permissions.map((perm) => {
                                    ...
                                    return (
                                      <span
                                        key={perm}
                                        className="rounded-full border border-teal-200 bg-teal-50 px-2 py-0.5 text-[11.5px] font-medium text-teal-600"
                                      >
                                        {p.label}
                                      </span>
                                    );
                            ...
                            <td className="px-5 py-4 whitespace-nowrap text-slate-500">{team.createdAt}</td>
                            <td className="px-5 py-4" onClick={(e) => e.stopPropagation()}>
                              {pendingDelTeamId === team.id ? (
                                <div className="flex items-center justify-end gap-1">
                                  ...
                                  <button
                                    onClick={() => setPendingDelTeamId(null)}
                                    className="rounded-lg p-1.5 text-slate-300 hover:bg-slate-100 hover:text-slate-500"
                                  >
                                    <X className="h-3.5 w-3.5" strokeWidth={2.5} />
                                  </button>
                                ...
                                <div className="flex items-center justify-end gap-2">
                                  ...
                                  <button
                                    onClick={() => setPendingDelTeamId(team.id)}
                                    className="rounded-lg p-1.5 text-slate-300 hover:bg-red-50 hover:text-red-400"
                                  >
                                    <Trash2 className="h-4 w-4" strokeWidth={2} />
                                  </button>
                        ...
                        <tr className={teams.length > 0 ? "border-t border-[#ebeef0]" : ""}>
                          <td className="px-5 py-3" colSpan={5}>
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                onClick={() => { setAddingTeam(false); setTeamNameInput(""); }}
                                className="rounded-lg p-1.5 text-slate-300 hover:bg-slate-100 hover:text-slate-500"
                              >
                                <X className="h-4 w-4" strokeWidth={2.5} />
                              </button>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div>
                ...
                {selectedUser ? (
                  <div className="flex items-center gap-3 rounded-xl border border-teal-300 bg-teal-50 px-4 py-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-[13.5px] font-semibold text-slate-800">{selectedUser.nickname}</p>
                      <p className="text-[12px] text-slate-500">{selectedUser.email}</p>
                    </div>
                    <button
                      onClick={() => setSelectedUser(null)}
                      className="rounded-lg p-1.5 text-slate-300 hover:bg-teal-100 hover:text-slate-500"
                    >
                      <X className="h-4 w-4" strokeWidth={2.5} />
                    </button>
                  </div>
              ...
              <div>
                <label className="mb-1.5 block text-[13px] font-medium text-slate-600">
                  소속 팀
                  <span className="ml-1.5 font-normal text-slate-400">(선택 · 팀 권한이 자동 적용됩니다)</span>
                </label>
                {selectedTeam ? (
                  <div className="flex items-center gap-3 rounded-xl border border-teal-300 bg-teal-50 px-4 py-3">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-teal-100">
