...
export async function getActiveMeetups(): Promise<Meetup[]> {
  ...
  const { data, error } = await supabase
    ...
    .select(
      `
      *,
      posts!inner ( title, user_id, status, created_at, public_profiles!posts_user_id_fkey ( name ) ),
...
export async function getMeetupDetail(postId: number): Promise<{
  ...
  const [meetupRes, participantsRes] = await Promise.all([
    supabase
      .from("meetups")
      .select(`*, posts!inner(title, user_id, status, public_profiles!posts_user_id_fkey(name))`)
      .eq("post_id", postId)
      .single(),
    supabase
      .from("meetup_participants")
      .select(`*, public_profiles(name, avatar_url, deleted_at)`)
