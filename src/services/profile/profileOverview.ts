import { createClient } from "@/utils/supabase/server";
import { getReviewsForUser } from "@/services/review/review";
import type { AlertSettings } from "@/hooks/profile/useAlertSettings";
import type { MeetupSummary } from "@/app/(user)/profile/_components/sections/ProfileMeetingsSection";
import { encryptPostId } from "@/utils/postIdCipher";
import { redirect } from "next/navigation";
import type { UserIdentity } from "@supabase/supabase-js";
import type { ReviewWithReviewer } from "@/type/review";

async function fetchAlertSettings(
  supabase: Awaited<ReturnType<typeof createClient>>,
  authId: string,
): Promise<{ alertSettings: AlertSettings; userId: number | null }> {
  const { data } = await supabase
    .from("users")
    .select(
      "id, alert_chat, alert_comment, alert_post, interest_categories, alert_keywords",
    )
    .eq("auth_id", authId)
    .single();

  const alertSettings: AlertSettings = {
    alert_chat: data?.alert_chat ?? true,
    alert_comment: data?.alert_comment ?? true,
    alert_post: data?.alert_post ?? false,
    interest_categories: data?.interest_categories ?? null,
    alert_keywords: data?.alert_keywords ?? null,
  };

  return { alertSettings, userId: data?.id ?? null };
}

async function fetchReviews(userId: number | null): Promise<ReviewWithReviewer[]> {
  return userId ? getReviewsForUser(userId) : [];
}

async function fetchCounts(
  supabase: Awaited<ReturnType<typeof createClient>>,
  userId: number | null,
): Promise<{ postCount: number; likeCount: number }> {
  if (!userId) return { postCount: 0, likeCount: 0 };

  const [postCount, likeCount] = await Promise.all([
    supabase
      .from("posts")
      .select("id", { count: "exact", head: true })
      .eq("user_id", userId)
      .eq("status", "active")
      .in("post_type", ["used_goods", "jobs", "rental"])
      .then(({ count }) => count ?? 0),
    supabase
      .from("common_likes")
      .select("target_id, posts!inner(id)", { count: "exact", head: true })
      .eq("user_id", userId)
      .eq("target_type", "post")
      .eq("posts.status", "active")
      .in("posts.post_type", ["used_goods", "jobs", "rental"])
      .then(({ count }) => count ?? 0),
  ]);

  return { postCount, likeCount };
}

type MeetupRow = {
  post_id: number;
  topic: string;
  start_at: string;
  end_at: string;
  location_address: string;
  max_participants: number;
  posts: {
    title: string;
    status: string;
    user_id: number;
    users?: { name: string | null } | null;
  };
  meetup_participants?: { status: string }[];
};

type JoinedRow = {
  meetup_post_id: number;
  meetups: MeetupRow;
};

function toCreatedMeetupSummary(r: MeetupRow): MeetupSummary {
  return {
    post_id: r.post_id,
    id_token: encryptPostId(r.post_id),
    title: r.posts.title,
    topic: r.topic,
    start_at: r.start_at,
    end_at: r.end_at,
    location_address: r.location_address,
    max_participants: r.max_participants,
    post_status: r.posts.status,
    participant_count: (r.meetup_participants ?? []).filter(
      (p) => p.status === "joined",
    ).length,
  };
}

function toJoinedMeetupSummary(r: JoinedRow): MeetupSummary {
  return {
    post_id: r.meetups.post_id,
    id_token: encryptPostId(r.meetups.post_id),
    title: r.meetups.posts.title,
    topic: r.meetups.topic,
    start_at: r.meetups.start_at,
    end_at: r.meetups.end_at,
    location_address: r.meetups.location_address,
    max_participants: r.meetups.max_participants,
    post_status: r.meetups.posts.status,
    participant_count: (r.meetups.meetup_participants ?? []).filter(
      (p) => p.status === "joined",
    ).length,
    host_name: r.meetups.posts.users?.name ?? undefined,
  };
}

const CREATED_MEETUPS_SELECT = `post_id, topic, start_at, end_at, location_address, max_participants,
  posts!inner(title, status, user_id),
  meetup_participants(status)` as const;

const JOINED_MEETUPS_SELECT = `meetup_post_id,
  meetups!inner(post_id, topic, start_at, end_at, location_address, max_participants,
    posts!inner(title, status, user_id, users!posts_user_id_fkey(name)),
    meetup_participants(status))` as const;

async function fetchCreatedMeetups(
  supabase: Awaited<ReturnType<typeof createClient>>,
  userId: number,
): Promise<MeetupSummary[]> {
  const { data } = await supabase
    .from("meetups")
    .select(CREATED_MEETUPS_SELECT)
    .eq("posts.user_id", userId)
    .order("start_at", { ascending: false });

  return ((data ?? []) as unknown as MeetupRow[]).map(toCreatedMeetupSummary);
}

async function fetchJoinedMeetups(
  supabase: Awaited<ReturnType<typeof createClient>>,
  userId: number,
): Promise<MeetupSummary[]> {
  const { data } = await supabase
    .from("meetup_participants")
    .select(JOINED_MEETUPS_SELECT)
    .eq("user_id", userId)
    .eq("status", "joined");

  return ((data ?? []) as unknown as JoinedRow[])
    .filter((r) => r.meetups.posts.user_id !== userId)
    .sort(
      (a, b) =>
        new Date(b.meetups.start_at).getTime() -
        new Date(a.meetups.start_at).getTime(),
    )
    .map(toJoinedMeetupSummary);
}

async function fetchMeetups(
  supabase: Awaited<ReturnType<typeof createClient>>,
  userId: number | null,
): Promise<{
  createdMeetups: MeetupSummary[];
  joinedMeetups: MeetupSummary[];
}> {
  if (!userId) return { createdMeetups: [], joinedMeetups: [] };

  const [createdMeetups, joinedMeetups] = await Promise.all([
    fetchCreatedMeetups(supabase, userId),
    fetchJoinedMeetups(supabase, userId),
  ]);

  return { createdMeetups, joinedMeetups };
}

export async function profileOverview(): Promise<{
  alertSettings: AlertSettings;
  identities: UserIdentity[];
  reviews: ReviewWithReviewer[];
  initialIsVerified: boolean;
  postCount: number;
  likeCount: number;
  createdMeetups: MeetupSummary[];
  joinedMeetups: MeetupSummary[];
}> {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user) redirect("/login");

  const { alertSettings, userId } = await fetchAlertSettings(supabase, user.id);
  const initialIsVerified = user.user_metadata?.identity_verified === true;

  const [reviews, counts, meetups] = await Promise.all([
    fetchReviews(userId),
    fetchCounts(supabase, userId),
    fetchMeetups(supabase, userId),
  ]);

  return {
    alertSettings,
    identities: user.identities ?? [],
    reviews,
    initialIsVerified,
    postCount: counts.postCount,
    likeCount: counts.likeCount,
    createdMeetups: meetups.createdMeetups,
    joinedMeetups: meetups.joinedMeetups,
  };
}

export type ProfileOverviewData = Awaited<ReturnType<typeof profileOverview>>;
