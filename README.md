...
기여도는 **커밋 수가 아니라, 각자 담당한 기능 영역(아래 [조원별 기여](#조원별-기여) 표의 각 행)의 "규모"와 "구현 난이도"를 함께 채점**해 산정했습니다.

**산정 기준**
1. [조원별 기여](#조원별-기여) 표의 한 행을 하나의 "기능 영역"으로 본다.
2. 두 가지 축으로 각각 **상(3점) · 중(2점) · 하(1점)** 을 매긴다 — 각 표의 `규모`·`난이도` 열에 근거를 표시했다.
   - **규모**: 코드가 걸치는 범위(DB·API·UI를 아우르는 정도, 페이지 하나인지 전 서비스인지)
   - **난이도**: 문제 자체의 기술적 난이도 — 동시성/Realtime 동기화, 결제·보안처럼 실수하면 안 되는 영역, 서드파티 API 연동의 까다로움, 참고할 기존 패턴 없이 처음부터 설계해야 했는지 여부
3. `총점 = 규모 + 난이도` (2~6점)로 영역별 점수를 낸다.
4. 한 영역을 여러 명이 함께 만든 경우(예: 안전결제 = 임태형 + 박소유) 해당 인원 각자의 표에 동일하게 나타나며, 점수를 나눠 갖지 않고 각자 자신의 표에 있는 점수를 그대로 받는다 — 공동 작업일수록 관련자 전원의 점수가 함께 올라간다.
5. 팀원별 총점 합계를 전체 합계로 나눠 비율을 낸다.

| 팀원 | 담당 기능 영역 수 | 총점 합계 (규모+난이도) | 기여도 |
| ---- | -----------------: | ------------------------: | -----: |
| 김도혁 | 10 | 44 | 30.8% |
| 박소유 | 7 | 31 | 21.7% |
| 이동근 | 9 | 35 | 24.5% |
| 임태형 | 7 | 33 | 23.1% |
| **합계** | **33** | **143** | **100%**(반올림 오차 ±0.1%p) |

> 규모·난이도 점수는 정성적 판단(코드 조사·회의록·WBS 근거)에 기반한 추정치이며, 절대적인 성과 서열이 아닙니다. 순수 규모만 봤을 때보다 임태형(안전결제·칸토고·관리자 아키텍처를 처음부터 설계)의 비중이 올라가는데, 이는 커밋 수는 적어도 기술적으로 까다로운 영역을 전담했기 때문입니다. 근거는 각 팀원 표의 `규모`·`난이도` 열과, 별도 활동 보고서(Git 커밋 분류·PR 목록·회의록·WBS)에서 확인할 수 있습니다.
