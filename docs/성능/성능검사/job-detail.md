...
[job-detail-지도지연·로고·쿼리.md](../성능개선/job-detail-지도지연·로고·쿼리.md) 개선(지도 지연 로딩·로고 next/image·쿼리 병렬화) 배포 후 재측정. **PSI 웹이 아니라 `docs/성능/scripts/psi-check.mjs`(로컬 Lighthouse)로 측정** — 방법론은 동일(Lighthouse 13.4.0)하지만 이 실행 환경은 CPU가 가상화돼 있어 모바일 스로틀링 결과의 실행 간 편차가 실제 PSI보다 훨씬 크다는 게 확인된 상태다(별도 검증 실행에서 같은 페이지가 5.7s↔9.8s로 나온 적 있음). 아래 모바일 수치는 참고용으로만 취급한다.

| 지표 | 📱 개선 전(PSI) | 📱 측정1 | 📱 측정2 | 📱 중간값 | 🖥️ 개선 전(PSI) | 🖥️ 측정1 | 🖥️ 측정2 | 🖥️ 중간값 |
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Perf | 84 | 67 | 57 | 62 | 83 | 100 | 100 | **100** |
| LCP | 3.0s 🟡 | 3.87s | 7.95s | 5.91s 🔴 | 0.8s 🟢 | 0.77s | 0.77s | **0.77s 🟢** |
| TBT | 360ms | 880ms | 652ms | 766ms | 340ms | 13ms | 10ms | **11.5ms** |
| CLS | 0 | 0.001 | 0 | ~0 | 0.026 | 0 | 0 | 0 |
| SI | 3.8s | 2.74s | 4.06s | 3.4s | 1.4s | 0.69s | 0.63s | 0.66s |

**🖥️ 데스크탑은 뚜렷한 개선이 확인됨**: Perf 83→100, TBT 340ms→11.5ms(지도 SDK가 초기 로드에서 빠진 효과와 정확히 일치), SI도 개선. 이 결과는 로컬 도구의 노이즈로 설명되지 않는 큰 폭이라 신뢰할 수 있다.

**📱 모바일은 판단 보류**: Perf·LCP·TBT 모두 개선 전보다 나빠 보이지만, 두 측정 자체가 3.87s↔7.95s로 2배 이상 벌어져 있어 — 이 페이지 고유의 회귀인지 로컬 환경의 CPU 스로틀링 노이즈인지 이 데이터만으로는 구분할 수 없다. `usedgoods` 목록 회귀(실제 PSI 웹으로 2회 확인해 확정)와 달리 이번 건은 **로컬 Lighthouse 결과만 있고 실제 PSI 재확인이 안 된 상태**이므로 회귀로 단정하지 않는다.

### 다음 확인 사항

- 모바일은 실제 PSI 웹(pagespeed.web.dev)으로 재확인 필요 — 로컬 결과만으로 "악화"로 기록하지 않는다.
- 데스크탑 개선(TBT 340→11.5ms)은 지도 지연 로딩이 의도대로 동작한다는 강한 신호로 봐도 됨.
