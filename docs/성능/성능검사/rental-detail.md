...
[rental-detail-쿼리병렬화.md](../성능개선/rental-detail-쿼리병렬화.md) 개선(좋아요·신고 쿼리 병렬화) 배포 후 재측정.

| 지표 | 📱 개선 전 | 📱 측정1 | 📱 측정2 | 📱 중간값 | 🖥️ 개선 전 | 🖥️ 측정1 | 🖥️ 측정2 | 🖥️ 중간값 |
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Perf | 86 | 65 | 62 | 63.5 | 98 | 98 | 97 | **97.5** |
| LCP | 2.8s 🟡 | 4.69s | 4.97s | **4.83s 🔴** | 0.9s 🟢 | 0.91s | 1.0s | 0.95s 🟢 |
| TBT | 370ms | 707ms | 847ms | **777ms** | 100ms | 27ms | 37ms | **32ms** |
| SI | 3.4s | 2.97s | 2.59s | 2.78s | 1.1s | 1.45s | 1.42s | 1.44s |

**📱 모바일 — 다른 페이지보다 신뢰도 있는 회귀 신호**: 두 측정(4.69s, 4.97s)이 서로 0.28s밖에 차이 나지 않아 — job-detail·job·main처럼 실행마다 크게 흔들리던 것과 달리 이번엔 일관됨. **LCP가 2.8s 🟡 → 4.83s 🔴로 등급이 떨어졌고 TBT도 370→777ms로 두 배 넘게 늘었다.** 이 페이지의 개선 작업(쿼리 병렬화)은 서버 렌더 단계라 클라이언트 LCP/TBT에 직접 영향을 줄 이유가 없어, `usedgoods`·`main`에서 공통으로 관찰된 "미사용 JS 증가" 추세와 같은 전역 요인(예: Sentry 등 공통 번들 변화)의 영향일 가능성이 있다.

**🖥️ 데스크탑**: TBT 100→32ms로 개선, LCP·Perf는 거의 유지.

### 다음 확인 사항

- 실제 PSI 웹으로 모바일 재확인 — 이번엔 로컬 두 측정이 서로 가까워서 신뢰도가 높은 편이지만, 그래도 최종 확정은 실제 PSI로
- 이 페이지의 쿼리 병렬화 개선과 무관해 보이는 회귀이므로, `usedgoods` 회귀 원인 조사와 같은 트랙에서 "공통 요인"(전역 번들 증가 등)부터 확인
