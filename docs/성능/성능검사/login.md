...
## 재측정 (배포 후) — 2026-07-06, 로컬 Lighthouse CLI (각 2회)

| 지표 | 📱 개선 전 | 📱 중간값 | 변화 | 🖥️ 개선 전 | 🖥️ 중간값 | 변화 |
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| Perf | 75 | 68 | 악화 | 85 | **97** | 개선 |
| LCP | 5.0s 🔴 | 4.39s 🔴 | 소폭 개선, 여전히 🔴 | 4.3s 🔴 | **1.07s 🟢** | **등급 2단계 상승** |
| TBT | 240ms | 608.5ms | 악화 | 100ms | 25.5ms | 개선 |
| SI | 3.0s | 2.26s | 개선 | 1.2s | 0.96s | 개선 |

**🖥️ 데스크탑 — 확정 개선**: LCP 4.3s 🔴 → 1.07s 🟢로 두 단계나 등급이 올랐다. 로그인 페이지 데스크탑은 서비스 전체에서 가장 뚜렷한 개선 사례 중 하나.
**📱 모바일**: LCP는 소폭 개선됐지만 여전히 🔴, TBT는 오히려 악화 — 다른 페이지들에서도 반복된 패턴(미사용 JS 496KiB, 원래 대비 증가 추정).
