...
## 청소년 보호정책 페이지 `/terms/youth`

- **대상 URL**: https://kanto-iota.vercel.app/terms/youth
- **PSI 링크 (📱 모바일)**: https://pagespeed.web.dev/analysis/https-kanto-iota-vercel-app-terms-youth/0olea862bd?form_factor=mobile
- **PSI 링크 (🖥️ 데스크탑)**: https://pagespeed.web.dev/analysis/https-kanto-iota-vercel-app-terms-youth/0olea862bd?form_factor=desktop
...
| :------: | :------------------: | ---- | ---- | ------ | :--: |
|    P0    |        모바일        | LCP 5.0s 🔴 | 렌더링 차단(600ms), Notion CMS 콘텐츠 로딩 지연 | 렌더링 차단 제거, 약관 콘텐츠 SSG/ISR 캐싱 적용 | 예정 |
|    P1    |        모바일        | TBT 220ms 🟡 | 긴 작업 5개, JS 블로킹 | 코드 스플리팅, 불필요 스크립트 지연 로딩 | 예정 |
|    P1    |        공통          | 미사용 JS ~186KiB | 불필요한 번들 포함 | Tree-shaking, dynamic import | 예정 |
|    P2    |        공통          | 레거시 JavaScript (15KiB) | 구형 폴리필 포함 | 레거시 폴리필 제거 | 예정 |

### 메모

- 접근성·권장사항 **100점 만점** — policy 페이지와 동일하게 우수
- 데스크탑 성능 96, LCP 1.0s 안정적
- 모바일 LCP(5.0s) 🔴 — policy(4.8s)와 동일한 패턴, Notion CMS 로딩이 원인
- policy.md와 구조 동일 — 두 페이지 함께 SSG 적용 시 동시 개선 가능
