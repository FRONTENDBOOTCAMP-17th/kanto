...
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| Perf | 76 | 56 | 악화 | 99 | 90 | 악화 |
| LCP | 4.9s 🔴 | 6.2s 🔴 | 악화 | 1.0s 🟢 | 1.55s 🟢 | 악화, 등급은 유지 |
| TBT | 290ms | 869.5ms | 크게 악화 | 40ms | 89ms | 악화 |
| SI | 2.1s | 3.41s | 악화 | 0.7s | 1.25s | 악화 |

**모바일·데스크탑 모두 악화**— login과 달리 이 페이지는 데스크탑도 전 지표가 나빠졌다(단, 등급은 여전히 🟢). `/login`과 `/signup`은 같은 회원 관련 플로우인데 정반대 결과라, 이 페이지 자체의 최근 변경이나 측정 시점 이슈(다른 트래픽/배포 상태)를 확인해볼 필요가 있다.
