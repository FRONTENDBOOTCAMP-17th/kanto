...
## 재측정 (배포 후) — 2026-07-06, 로컬 Lighthouse CLI (각 2회, 두 측정 근접해 신뢰도 있음)

[terms-SSR전환.md](../성능개선/terms-SSR전환.md) 배포 후 재측정.

| 지표 | 📱 개선 전 | 📱 측정1 | 📱 측정2 | 📱 중간값 | 🖥️ 개선 전 | 🖥️ 중간값 |
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| Perf | 72 | 58 | 63 | 60.5 | 96 | **100** |
| LCP | 5.1s 🔴 | 7.14s | 7.39s | **7.26s 🔴** | 1.0s 🟢 | **0.73s 🟢** |
| TBT | 390ms | 638ms | 497ms | 567.5ms | 150ms | **6ms** |
| SI | 2.0s | 4.08s | 2.44s | 3.26s | 0.7s | 0.52s |

**🖥️ 데스크탑 — 확정 개선**(SSR+ISR 효과). **📱 모바일 — 두 측정(7.14s/7.39s)이 근접해 신뢰도 있는 악화**: LCP 5.1s→7.26s로 오히려 나빠짐. `TermsClientWrapper` 제거(클라이언트 fetch→서버 fetch)는 원리상 LCP를 개선해야 하는데 실제로는 반대라, 서버 측 Notion API 응답 자체가 느려졌거나(캐시 미스), ISR revalidate 설정이 기대대로 동작하지 않을 가능성을 의심해봐야 한다.
