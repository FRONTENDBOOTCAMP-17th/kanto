...
## 재측정 (배포 후) — 2026-07-06, 각 2회 평균 (16:53/16:54 캡처, 중복 리포트 제외)

| 지표 | 📱 개선 전 | 📱 개선 후(평균) | 변화 | 🖥️ 개선 전 | 🖥️ 개선 후(평균) | 변화 |
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| Perf | 81 | 79.5 | −1.5 | 88 | 84.5 | −3.5 |
| LCP | 3.1s 🟡 | 3.75s 🟡 | +0.65s (등급 유지) | 0.8s 🟢 | 0.7s 🟢 | −0.1s (개선) |
| TBT | 430ms | 345ms | −85ms (개선) | 270ms | 325ms | +55ms (악화) |
| SI | 4.0s | 4.2s | +0.2s | 1.2s | 1.55s | +0.35s |

> 이 페이지는 공통-LCP 개선(usedgoods/rental/main 목록 전용) 대상이 아니라 뚜렷한 개선·악화 신호 없이 대체로 변동 범위 안. `/usedgoods` 목록의 모바일 LCP 급등(→8.3s)과 달리 이 상세 페이지는 정상 범위.
