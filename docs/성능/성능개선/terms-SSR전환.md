...
- **우선순위**: P0
- **상태**: 🔀 데스크탑은 확정 완료 · 모바일은 확정 회귀 (§3 참고)
...
> 2026-07-06 로컬 Lighthouse(각 2회) 재측정 — 대상 5페이지 전체. 상세는 각 검사 문서 §재측정 참고: [policy](../성능검사/policy.md#재측정-배포-후--2026-07-06-로컬-lighthouse-cli-자동-측정-각-2회) · [termsservice](../성능검사/termsservice.md#재측정-배포-후--2026-07-06-로컬-lighthouse-cli-각-2회-두-측정-근접해-신뢰도-있음) · [privacy](../성능검사/privacy.md#재측정-배포-후--2026-07-06-로컬-lighthouse-cli-각-2회-모바일-편차-매우-큼--참고치) · [youth](../성능검사/youth.md#재측정-배포-후--2026-07-06-로컬-lighthouse-cli-각-2회-두-측정-근접해-신뢰도-있음) · [payment](../성능검사/payment.md#재측정-배포-후--2026-07-06-로컬-lighthouse-cli-각-2회-두-측정-근접해-신뢰도-있음)

### 🖥️ 데스크탑 — 5페이지 전부 확정 개선

| 페이지 | Perf 전→후 | LCP 전→후 | TBT 전→후 |
|---|:---:|:---:|:---:|
| policy | 98→100 | 1.0s→0.73s 🟢 | 90→7.5ms |
| service | 96→100 | 1.0s→0.73s 🟢 | 150→6ms |
| privacy | 93→100 | 1.0s→0.74s 🟢 | 180→15.5ms |
| youth | 96→100 | 1.0s→0.73s 🟢 | 140→12.5ms |
| payment | 98→100 | 0.9s→0.7s 🟢 | 90→23ms |

### 📱 모바일 — 5페이지 중 3곳 확정 악화, 1곳 판단 불가(policy), 1곳 편차 큼(privacy)

| 페이지 | Perf 전→후 | LCP 전→후 | 신뢰도 |
|---|:---:|:---:|---|
| policy | 81→67.5 | 4.8s→5.33s | 판단 불가 (두 측정 3.44/7.21s로 3.77초 벌어짐) |
| service | 72→60.5 | 5.1s→**7.26s** | 두 측정 근접(7.14/7.39s) — **신뢰도 있는 악화** |
| privacy | 73→66 | 4.9s→5.31s | 편차 큼 (3.42/7.21s) — 참고치 |
| youth | 78→58 | 5.0s→**7.14s** | 두 측정 근접(7.02/7.26s) — **신뢰도 있는 악화** |
| payment | 78→58 | 4.9s→**7.17s** | 두 측정 근접(7.14/7.20s) — **신뢰도 있는 악화** |

> service·youth·payment 세 페이지가 모바일 LCP 7.1~7.3s대로 나란히 수렴 — 이 개선 자체(SSR+ISR)의 문제라기보다 **약관 5페이지가 공유하는 무언가**(Notion API 응답 지연, 또는 다른 페이지에서도 관찰된 전역 번들 증가)가 원인일 가능성이 높다.

**결론**: 🖥️ 데스크탑은 완료로 확정. 📱 모바일은 최소 3/5 페이지에서 확정 회귀 — 이 개선 전체를 "완료"로 보기 어렵다.
...
- **재측정 (2026-07-06, 로컬 Lighthouse 각 2회, 5페이지 전체)**: §3 참고
- **개선 확인 결과**: 🖥️ 데스크탑 5페이지 모두 확정 개선(SSR+ISR 효과 뚜렷). 📱 모바일은 service·youth·payment 3페이지가 LCP 5s대→7s대로 확정 악화 — 원인은 SSR 전환 자체보다 5페이지 공통 요인(Notion API 응답 또는 전역 번들 증가)으로 추정되며 별도 조사 필요. 실제 PSI로 최종 확인 전까지 이 문서를 완전한 "완료"로 보지 않는다.
