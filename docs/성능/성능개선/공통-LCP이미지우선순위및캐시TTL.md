...
### 3-1. PSI (배포 URL 기준) — 2026-07-06 재측정 결과

> `usedgoods`·`main`은 실제 PSI 웹(2회 측정 평균), `rental`은 로컬 Lighthouse CLI(`docs/성능/scripts/psi-check.mjs`, 각 2회 중간값 — 모바일은 참고치)로 측정. 상세는 각 `성능검사/*.md`의 "재측정" 절 참고.
> ⚠️ 이 표의 "개선 전" LCP(usedgoods 4.4s/6.1s)는 초안 작성 시점의 값이며, 실제 검사 문서([usedgoods.md](../성능검사/usedgoods.md))에 기록된 값(2.6s/0.7s)과 다르다. 아래는 검사 문서 기준으로 정정한 값이다.

| 지표             |  디바이스   |  개선 전 (검사 문서 기준)  |  개선 후   | 변화 |
| ---------------- | :---------: | :--------: | :--------: | :--: |
| LCP (usedgoods)  |  📱 모바일  |   2.6s 🟡  |  **8.3s 🔴 (실제 PSI, 2회 확정)** | **+5.7s, 회귀** |
| LCP (usedgoods)  | 🖥️ 데스크탑 |   0.7s 🟢  |  0.9s 🟢 (실제 PSI, 2회) | +0.2s, 유지 |
| LCP (main)       |  📱 모바일  |   4.7s 🔴  |  3.45s 🟡 (실제 PSI, 2회) | −1.25s, 개선 |
| LCP (main)       | 🖥️ 데스크탑 |   0.6s 🟢  |  0.75s 🟢 (실제 PSI, 2회) | +0.15s, 유지 |
| LCP (rental)     |  📱 모바일  |   11.3s 🔴 | 7.48s 🔴 (로컬 도구, 참고치) | −3.82s, 개선되나 목표 미달 |
| LCP (rental)     | 🖥️ 데스크탑 |   2.8s 🟡  |  **1.05s 🟢 (로컬 도구)** | −1.75s, 등급 상승 |
| Improve image delivery (usedgoods) | 모바일/데스크탑 | 2,567 KiB | (재측정 시 별도 확인 필요 — 자동 스크립트는 이미지 절감치 KiB를 집계하지 않음) | — |

> **종합**: 데스크탑 3곳(usedgoods/main/rental) 모두 🟢 유지 또는 개선. 모바일은 main·rental은 개선(다만 rental은 여전히 🔴), **usedgoods만 유일하게 확정 회귀** — 세 페이지가 같은 `ContentCard` 공용 컴포넌트를 쓰는데 usedgoods에서만 악화된 원인을 먼저 찾아야 한다.
...
- **재측정 (2026-07-06)**: usedgoods·main은 실제 PSI 웹 2회, rental은 로컬 Lighthouse 2회 — 결과는 §3-1 표 참고
...
- **개선 확인 결과**: 데스크탑은 usedgoods/main/rental 3곳 모두 🟢 유지·개선으로 검증 완료. 모바일은 main·rental은 개선(rental은 여전히 🔴), **usedgoods만 확정 회귀(2.6s🟡→8.3s🔴)** — 원인 조사 전까지 이 문서 전체를 완료로 표시하지 않는다.
